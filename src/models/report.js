import {
	...
	sign,
	remove,
	maidian,updateQuestionInfo,qrKnowledgeInfo,questionAddKnowledge,qrQuestionInfo,modifyQuestionAttr
} from '../services/reportService';
import {
	queryQuestionsBy,updateQuestion,recoverQuestion
...
export default {
	...
	reducers: {
		lastQueryOption(state, { payload }) {
			return { ...state, lastQueryOption: payload };
		},
	...
	effects: {
		...
		*qrKnowledgeInfo({ payload }, { put, select }) {
			let zsdslist=[]
			let zres=yield  qrKnowledgeInfo(payload)
			console.log('zres: ', zres);
			if(zres&&zres.data.result===0){
				if(zres.data.data&&zres.data.data.knowledge&&zres.data.data.knowledge.length) zsdslist=zres.data.data&&zres.data.data.knowledge
			}else{
		...
		*getGradedata({ payload }, { put, select }) {
			// let res=yield recoverQuestion(payload)
			// return res
			let gdata=[
				{
			...
			return {
				value:gdata.length?gdata[0].value:-1,
				list:gdata
			}
		},
		
		*getSubjectData({ payload }, { put, select }) {
			let res=yield subjectNodeList(payload)
			if(res.data.result===0){
				let slist=res.data.data?res.data.data:[]
				if(slist.length){

					return {
						value:slist.length?slist[0].k:-1,
						list:slist
					}
				}
			}else{
		...
		*getClassData({ payload }, { put, select }) {
			// let res=yield recoverQuestion(payload)
			// return res
			let cdata=[
				{
			...
			return {
				value:cdata.length?cdata[0].value:-1,
				list:cdata
			}
		},
		...
		*printPicTrigger({payload}){
			let res=yield updateQuestionInfo(payload)
			console.log('payload: ', res);
			return res
		},
		*questionAddKnowledge({payload}){
			let res=yield questionAddKnowledge(payload)
			console.log('payload: ', res);
			return res
		},
		*qrQuestionInfo({payload}){
			let res=yield qrQuestionInfo(payload)
			console.log('payload: ', res);
			return res
		},
		*modifyQuestionAttr({payload}){
			let res=yield modifyQuestionAttr(payload)
			console.log('modifyQuestionAttr: ', res);
			return res
		},
	},
